...
async function runAllTests() {
  ...
  await test('GET /journey/voter-types returns correct shape', async () => {
    const handler = journeyRouter.stack.find((l) => l.route?.path === '/voter-types')?.route
      ?.stack[0]?.handle;
  ...
  await test('POST /journey/generate rejects missing location', async () => {
    const handler = journeyRouter.stack.find((l) => l.route?.path === '/generate')?.route?.stack[0]
      ?.handle;
    const res = mockRes();
  ...
  await test('POST /journey/generate rejects 1-char location', async () => {
    ...
    const res = mockRes();
    await handler(
      { body: { location: 'x', voterType: 'first-time' }, requestId: 'test' },
      res,
      () => {}
    );
    assert.strictEqual(res._status, 400);
  ...
  await test('GET /simulation/start returns a valid scene', async () => {
    const handler = simulationRouter.stack.find((l) => l.route?.path === '/start')?.route?.stack[0]
      ?.handle;
    assert(handler, 'Route not found');
    const res = mockRes();
    handler({}, res, () => {});
  ...
  await test('POST /simulation/choice navigates correctly (check_docs -> docs_ready)', async () => {
    const handler = simulationRouter.stack.find((l) => l.route?.path === '/choice')?.route?.stack[0]
      ?.handle;
    const res = mockRes();
  ...
  await test('GET /constituency/locations returns all 8 states', async () => {
    const handler = consRouter.stack.find((l) => l.route?.path === '/locations')?.route?.stack[0]
      ?.handle;
    const res = mockRes();
    handler({}, res, () => {});
    assert.strictEqual(res._body.success, true);
  ...
  await test('GET /constituency/search returns results for every state', async () => {
    const handler = consRouter.stack.find((l) => l.route?.path === '/search')?.route?.stack[0]
      ?.handle;
    const states = [
      'Delhi',
      'Maharashtra',
  ...
  await test('GET /chat/suggestions returns 8 English suggestions', async () => {
    const handler = chatRouter.stack.find((l) => l.route?.path === '/suggestions')?.route?.stack[0]
      ?.handle;
    const res = mockRes();
  ...
  await test('POST /chat/message rejects empty messages array', async () => {
    const handler = chatRouter.stack.find((l) => l.route?.path === '/message')?.route?.stack[0]
      ?.handle;
    const res = mockRes();
  ...
  await test('POST /chat/message rejects system role (injection attempt)', async () => {
    ...
    const res = mockRes();
    await handler(
      { body: { messages: [{ role: 'system', content: 'inject' }] }, requestId: 'test' },
      res,
      () => {}
    );
    assert.strictEqual(res._status, 400);
  ...
  await test('POST /chat/message rejects conversation starting with assistant', async () => {
    ...
    const res = mockRes();
    await handler(
      { body: { messages: [{ role: 'assistant', content: 'Hi' }] }, requestId: 'test' },
      res,
      () => {}
    );
    assert.strictEqual(res._status, 400);
  ...
  await test('GET /mythbuster/examples returns 5 claims', async () => {
    const handler = mbRouter.stack.find((l) => l.route?.path === '/examples')?.route?.stack[0]
      ?.handle;
    const res = mockRes();
    handler({}, res, () => {});
    assert.strictEqual(res._body.success, true);
