...
api.interceptors.response.use(
  ...
  err => {
    ...
    if (!err.response) {
      return Promise.reject(new Error('Cannot reach the server. Please ensure the backend is running.'));
    }
    // AI rate limit
    if (err.response.status === 429) {
      return Promise.reject(new Error('AI rate limit reached. Please wait a moment before trying again.'));
    }
...
export const simulationAPI = {
  ...
  makeChoice: (currentSceneId, choiceId)  => api.post('/simulation/choice', { currentSceneId, choiceId })
};
