            ...
            <div className="mythbuster-input-card card">
              <div className="mythbuster-input-header">
                <div className="mythbuster-input-icon" aria-hidden="true">
                  <svg width="20" height="20" viewBox="0 0 20 20" fill="none" aria-hidden="true">
                    <path
                      d="M10 2l2 6h6l-4.8 3.5 1.8 6L10 14l-5 3.5 1.8-6L2 8h6l2-6z"
                      stroke="currentColor"
                      strokeWidth="1.4"
                      strokeLinecap="round"
                      strokeLinejoin="round"
                ...
                <div>
                  ...
                  <p className="text-small text-muted">
                    Paste any election-related claim to verify its accuracy
                  </p>
                </div>
              </div>
                ...
                >
                  {loading ? (
                    ...
                    <>
                      ...
                      >
                        <path
                          d="M7.5 1.5L9.5 6l5 .5-3.5 3.5 1 5L7.5 12.5l-4.5 2.5 1-5L.5 6.5l5-.5z"
                          stroke="currentColor"
                          strokeWidth="1.3"
                          strokeLinecap="round"
                          strokeLinejoin="round"
                ...
                {result && config && (
                  ...
                  >
                    <div className="mythbuster-verdict">
                      <div
                        className={`mythbuster-verdict-icon verdict-${config.color}`}
                        aria-hidden="true"
                      >
                        {config.icon}
                      </div>
                        ...
                        <div className="mythbuster-sources-list">
                          {result.sources.map((s, i) => (
                            <div key={`src-${i}`} className="mythbuster-source-item">
                              ...
                              >
                                <path
                                  d="M2 6.5C2 4.015 4.015 2 6.5 2M6.5 11C8.985 11 11 8.985 11 6.5"
                                  stroke="currentColor"
                                  strokeWidth="1.2"
                                  strokeLinecap="round"
                                />
                ...
                <div className="mythbuster-history-list">
                  {history.slice(1).map((h, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`mythbuster-history-dot dot-${c.color}`}
                          aria-hidden="true"
                        />
                        <div>
                          <div className="text-small">
                            {h.claim.substring(0, 70)}
                            {h.claim.length > 70 ? '...' : ''}
                          </div>
                          <div className={`text-xs verdict-text-${c.color}`}>
                            {h.result.verdict}
                          </div>
                        </div>
                      </button>
            ...
            <div className="card">
              ...
              <div className="mythbuster-examples">
                {examples.map((ex, i) => (
                  <button
                    key={`ex-${i}`}
                    className="mythbuster-example-btn"
              ...
              <div className="mythbuster-guide">
                {Object.entries(VERDICT_CONFIG).map(([verdict, cfg]) => (
                  <div key={verdict} className={`mythbuster-guide-item guide-${cfg.color}`}>
                    <div
                      className={`mythbuster-guide-icon verdict-${cfg.color}`}
                      aria-hidden="true"
                    >
                      {cfg.icon}
                    </div>
                    <div>
                      <div className={`text-small font-medium verdict-text-${cfg.color}`}>
                        {verdict}
                      </div>
                    </div>
                  </div>
