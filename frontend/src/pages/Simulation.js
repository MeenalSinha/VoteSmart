  ...
  const handleChoice = async (choice) => {
    ...
    try {
      ...
      setLastFeedback({
        score: 0,
        correct: false,
        feedback: 'Could not process your choice. Please try again.',
      });
    }
  };
        ...
        <div className="container">
          <div className="sim-intro card">
            ...
            <p className="text-muted">
              Navigate real-world polling scenarios. Make decisions, face consequences, and learn
              the right way to vote. Your choices determine the outcome.
            </p>
            ...
            <AnimatePresence mode="wait">
              {gameOver ? (
                ...
                >
                  ...
                  <p className="text-muted">
                    {scorePercent >= 80
                      ? 'Excellent! You are well-prepared for election day.'
                      : scorePercent >= 50
                        ? 'Good effort. Review the areas where you lost points.'
                  ...
                  <div className="sim-history">
                    ...
                    {history.map((h, i) => (
                      <div
                        key={i}
                        className={`sim-history-item ${h.correct ? 'correct' : h.score < 0 ? 'wrong' : 'partial'}`}
                      >
                        <div
                          className={`sim-history-badge badge badge-${h.correct ? 'green' : h.score < 0 ? 'red' : 'amber'}`}
                        >
                ...
                >
                  ...
                  {scene?.question && <div className="sim-scene__question">{scene.question}</div>}
                  ...
                  {lastFeedback && (
                    ...
                    >
                      ...
                      <span
                        className={`badge badge-${lastFeedback.correct ? 'green' : lastFeedback.score < 0 ? 'red' : 'amber'}`}
                      >
              ...
              <div className="sim-sidebar__list">
                {history.map((h, i) => (
                  <div key={i} className="sim-sidebar__item">
                    <div
                      className={`sim-sidebar__dot ${h.correct ? 'green' : h.score < 0 ? 'red' : 'amber'}`}
                    ></div>
                    <div>
